...
def fab_install_site(platform_name, profile):
        fabric.run("su - -s /bin/sh aegir -c '/var/aegir/drush/drush.php --uri=\'%s.spoontastic.info\' provision-save \'@%s.spoontastic.info\' --context_type=\'site\' --platform=\'@platform_%s\' --profile=\'%s\' --db_server=\'@server_localhost\''" % (platform_name, platform_name, platform_name, profile), pty=True)
        fabric.run("su - -s /bin/sh aegir -c '/var/aegir/drush/drush.php @%s.spoontastic.info provision-install'" % platform_name, pty=True)
...
def run_site_tests():
        ...
        fab_install_site('kickstarterclone', 'default')
